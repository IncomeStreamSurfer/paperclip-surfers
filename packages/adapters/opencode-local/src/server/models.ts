...
export async function discoverOpenCodeModels(input: {
  ...
  // If PAPERCLIP_HOME is set, use it to locate config files instead of os.userInfo()
  // This handles containerized environments where the running user's homedir differs
  // from where configs are mounted.
  let resolvedHome: string | undefined;
  const paperclipHome = process.env.PAPERCLIP_HOME;
  if (paperclipHome) {
    resolvedHome = paperclipHome;
