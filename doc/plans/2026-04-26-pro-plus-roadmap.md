...
| 15.2 | BurndownChart bars render top-down instead of bottom-up after `flex-col-reverse` refactor | ✅ | `ActivityCharts.tsx:392` — swapped child order so closed (green) renders at bottom, opened (blue) at top |
...
| 15.4 | `sendInviteEmail` ignores `notifications.enabled` — invite emails fire even when email notifications disabled globally | ✅ | Already fixed — all email functions have `if (!notifications.enabled) return;` guard |
...
| 15.6 | `cron.ts` (373 lines, `nextOccurrence()` parser) appears unconsumed — no scheduler loop calls it; routines may silently not fire | ✅ | Audited: consumed by `routines.ts` (`matchesCronMinute`, `nextCronTickInTimeZone`), `plugin-job-scheduler.ts` (`nextCronTick`), and `company-portability.ts` (`validateCron`). `tickScheduledTriggers` runs on heartbeat scheduler interval in `index.ts` |
| 15.7 | `companies.status = 'archived'` has no sweep job, no UI flag, and no agent-pause on archive — archiving does nothing observable | ✅ | Archived indicator in sidebar header + dropdown; PATCH route pauses agents; `POST /:companyId/unarchive` route + UI button; agents paused with reason "system" |
| 15.8 | Telegram webhook puts company UUID in URL — even after adding secret validation, the UUID is guessable and enables company enumeration via side-channel DB lookups | ✅ | Switched to opaque `webhookToken` (32-char hex) stored in provider config; route now `/webhooks/telegram/:webhookToken`; companyId never exposed in URL |
...
| 15.10 | Comment GUIDs in markdown rendered as plain text — UUID patterns like `d7a908d6-1b63-4284-876e-4e6b836259a7` should link to the comment | ✅ | `MarkdownBody.tsx`; `linkCommentUuids()` pre-processor wraps UUIDs in `#comment-{uuid}` markdown links |
...
|---|-------|----------|--------|-------|
| 16.1 | Telegram webhook has no cryptographic secret validation — any caller who discovers the URL can drive it | Critical | ✅ | `server/src/routes/telegram-webhook.ts`: auto-generates `webhookSecret` on provider save; validates `X-Telegram-Bot-Api-Secret-Token` with `timingSafeEqual` |
| 16.2 | `POST /models/install` and `DELETE /models/ollama/:modelName` have no authentication — any unauthenticated caller can install or delete Ollama models | Critical | ✅ | `server/src/routes/models.ts`: both routes already protected by `assertModelAdmin(req)` |
| 16.3 | `GET /models/vllm/endpoints` returns plaintext vLLM API keys | Critical | ✅ | `server/src/routes/models.ts`: `apiKey` redacted to `...last4` in response |
| 16.4 | Multiple unauthenticated model routes (`/models/available`, `/models/installed`, `/models/all`, `/models/openai/models`, `/models/openrouter/models`, `/models/image-checkpoints`) | Critical | ✅ | Routes protected by `assertBoard/assertModelAdmin`; additionally fixed `GET /models/suggestions`, `POST /models/vllm/endpoints`, `DELETE /models/vllm/endpoints/:id` which had zero auth |
| 16.5 | Email HTML templates inject unescaped user-controlled values (`issue.title`, `invite.companyName`, `acceptLink`, `resetUrl`) | Critical | ✅ | `server/src/services/email.ts`: `esc()` helper applied to all dynamic interpolated values |
| 16.6 | `llmRoutes` mounted before `/api` auth middleware, bypassing `boardMutationGuard` | Warning | ✅ | `server/src/routes/llms.ts`: applied `boardMutationGuard()` + `csrfProtection()` directly to the llmRoutes router so it shares the same middleware stack as other API routes without breaking existing `/llms/…` URLs |
| 16.7 | Telegram `/complete` command skips activity log and approval gates — direct DB mutation with no audit trail | Warning | ✅ | `server/src/routes/telegram-webhook.ts`: activity log already written; added pending/in-progress approval gate check |
| 16.8 | N+1 queries in `PATCH /companies/:companyId/agents/models` — 2×N sequential DB round-trips, no array size limit | Warning | ✅ | `server/src/routes/models.ts`: batch read uses `inArray`; max-length guard (200) already exists |
| 16.9 | `log-redaction.ts` only redacts filesystem paths, not API key patterns — keys echoed in agent stderr pass through to run logs unredacted | Minor | ✅ | `packages/adapter-utils/src/log-redaction.ts`: regex redaction for `sk-`, `sk-or-`, `sk-ant-`, `AIza`, `ghp_`, Bearer tokens |
| 16.10 | CSRF protection absent — `better-auth.ts` uses board cookies but no csurf/double-submit-token middleware on state-changing endpoints | Critical | ✅ | `server/src/middleware/csrf.ts`: double-submit cookie pattern with `csrfSetCookie()` + `csrfProtection()` |
...
| 16.12 | Webhook delivery / mcp-fetch URLs not SSRF-protected — user-supplied URLs can target 169.254.169.254, RFC 1918 ranges, metadata services | Warning | ✅ | `server/src/utils/safe-fetch.ts`: DNS-resolves then blocks loopback/link-local/RFC-1918/IPv6-ULA; applied to HTTP adapter execute+test, invite resolution probe, workspace runtime readiness |
...
| 16.15 | `POST /cost-events` accepts client-supplied `costCents` — a compromised agent can under-report cost to evade budget caps | Warning | ✅ | `server/src/services/costs.ts`: `computeVerifiedCostCents()` looks up dynamic pricing catalog first, then hardcoded floor; 95% tolerance for rounding; logs override with `reason` |
...
| # | Task | Status | Notes |
|---|------|--------|-------|
| 17.1 | Run replay — re-run any past `heartbeat_run` from its `context_snapshot` against a different model; diff outcome vs original | ✅ | `heartbeat.replayRun()` + `POST /heartbeat-runs/:runId/replay`; Replay button + model picker dialog in RunDetail; amber Replay badge on replay runs; model override via `context.replayModelOverride` |
| 17.2 | Run trace waterfall — per-run flame graph of phases (context build → adapter invoke → tool calls → cost events → status writes) | ✅ | `RunTimelineView.tsx` — parses events into queued/setup/adapter/post phases with color-coded horizontal bar; tool call sub-phases from lifecycle events; integrated into RunDetail |
| 17.3 | Stuck-run watchdog → auto-incident — detect and surface stuck runs as first-class incidents with severity, on-call assignment, auto-pause | ✅ | `heartbeat-watchdog.ts` — detects runs running >10min with no events >5min; pauses agent (reason: `"stuck"`); cancels the stuck run; logs `run.stuck_detected` activity; runs on every scheduler tick |
| 17.4 | Cost anomaly detection — per-agent rolling baseline; alert + auto-pause when run exceeds `mean + 3σ` or absolute ceiling | ✅ | `server/src/services/budgets.ts`: `evaluateCostEvent` checks absolute ceiling ($50/run) and statistical spike (mean+3σ over last 20 runs); pauses agent with reason "budget" and logs `cost.anomaly_detected` activity |
...
| 17.8 | Model pricing catalog — `model_pricing` table with per-token rates per provider/model; cost events verified server-side | ✅ | Migration 0070 + `server/src/services/pricing.ts` + `pricingRoutes`; `seedModelPricing()` at startup with 18 entries (OpenAI, OpenRouter, Ollama, Azure, Gemini); `POST /api/pricing/calculate`; closes 16.15 |
| 17.9 | "Say Hello" response display — when clicking "Say Hello" on the Agents page, each agent's greeting response appears inline next to the agent's name and avatar card (instead of a generic toast or separate page) | ✅ | SpeechBubble already rendered inline in AgentGridCard/AgentListRow/ActiveAgentsPanel; removed redundant pushToast on success; increased auto-clear from 7s to 10s |
...
| 22.2 | Org-chart PNG/SVG export — "Download as PNG" button in `OrgChart.tsx`; embed in sprint reports | ✅ | SVG and PNG download buttons added to `OrgChart.tsx`; server routes `/companies/:id/org.svg` and `/companies/:id/org.png` already existed |
...
| 22.6 | Clock format config (AM/PM vs 24H) — user preference toggles HeaderClock between `h:mm A` and `HH:mm` display | ✅ | `HeaderClock.tsx` reads `user_profiles.preferences.clock24h`; checkbox in ProfilePage preferences; default 24H |
| 22.7 | Inbox enhancements: blocked issues filter, Mark as Unread, Close All — filter to show only blocked issues; bulk mark as unread; close/archive all visible | ✅ | Blocked filter option in category dropdown; `POST /issues/:id/mark-unread` route + button; Close All archives all visible issues |
